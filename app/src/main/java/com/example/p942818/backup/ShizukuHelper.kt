...
object ShizukuHelper {
    ...
    private fun execWithShizukuShell(command: String): CommandResult {
        return runWithTimeout(60) {
            try {
                val service = getShellService()
                    ?: return@runWithTimeout CommandResult(-1, "", "无法连接Shizuku用户服务，请重新授权Shizuku")
                val result = service.exec(command)
                val idx = result.indexOf('\u0000')
                if (idx < 0) return@runWithTimeout CommandResult(-1, "", result)
        ...
        } ?: CommandResult(-1, "", "操作超时(60秒)，shell命令执行被卡住了")
    }
